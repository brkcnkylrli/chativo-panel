...
<template>
  ...
  >
    <!--
      Telefonda baslik ve kontroller alt alta.
      `auto-fit` + `max-content` dar ekranda ikisini yan yana zorluyor ve
      "Son 7 gun", "Tum Kanallar" gibi dugmeler kart sinirini asiyordu.
    -->
    <div
      class="card-header grid w-full mb-6 grid-cols-1 gap-y-3 sm:grid-cols-[repeat(auto-fit,minmax(max-content,50%))] sm:gap-y-2"
    >
      <slot name="header">
        ...
        <!-- Kontroller telefonda sola yaslanip sarmali; saga yaslanmis
             tek satir dar ekranda tasiyordu. -->
        <div
          class="flex flex-row flex-wrap items-center justify-start gap-2 sm:justify-end sm:flex-nowrap"
        >
          <slot name="control" />
        </div>
      </slot>
